...
def metric_calculation(filterfile, judgment_file):
    # Load the JSON file
    with open(filterfile, 'r') as file1:
        filter_datas = json.load(file1)
    ...
    for item, filter_data in zip(data, filter_datas):
        total += 1
        judgement = item[0]
        if "NONE" in filter_data:
            incorrect_count += 1
        elif "INCONCLUSIVE" in judgement:
...
right_res = metric_calculation('./qa/filter_hallucination/Starling-LM-7B-alpha/right.json',
                               './qa/judgment/Starling-LM-7B-alpha/right_ground_triplet.json')
hallucinated_res = metric_calculation('./qa/filter_hallucination/Starling-LM-7B-alpha/hallucinated.json',
                                      './qa/judgment/Starling-LM-7B-alpha/hallucinated_ground_triplet.json')
